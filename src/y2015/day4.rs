...
fn md5(input: &[u8]) -> [u8; 16] {
    ...
    digest.extend(&a0.to_le_bytes());
    digest.extend(&b0.to_le_bytes());
    digest.extend(&c0.to_le_bytes());
    digest.extend(&d0.to_le_bytes());

    let mut arr = [0; 16];
    for (place, data) in arr.iter_mut().zip(digest.iter()) {
        *place = *data;
    }
    arr
}
